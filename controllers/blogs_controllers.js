...
blogsRouter.post('/', async (request, response) => {
  ...
  if (!title || !author || !url) {
   ...
   // Verificamos que el usuario autenticado existe en request.user
   if (!request.user) {
...
blogsRouter.delete('/:id', async (request, response) => {
  ...
  // Verificamos si el ID es válido antes de consultar la base de datos
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return response.status(400).json({ error: 'malformatted id' })
  }
  ...
  // Comparamos el usuario del blog con el usuario autenticado
  if (blog.user.toString() !== request.user._id.toString()) {
    return response.status(403).json({ error: 'No tienes permiso para eliminar este blog' })
  }
