// Controlador de Ruta para Usuarios
...
usersRouter.get('/', async (request, response) => {
  ...
  const users = await User.find({}).populate('blogs', { title: 1, author: 1, url: 1, like: 1 }) // Método populate para la unión Mongoose entre users y notes -- Mostrando solo los campos content e important
...
usersRouter.post('/', async (request, response) => {
  ...
  // Verificación de existencia y longitud de username y password
  if (!username || !password) {
    return response.status(400).json({ error: 'Username y password son requeridos' })
  }
  if (username.length < 3 || password.length < 3) {
    return response.status(400).json({ error: 'Username y password deben tener al menos 3 caracteres' })
  }
